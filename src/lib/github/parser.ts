...
export function inferCategory(
  ...
  // RAG related
  if (combined.includes('rag') || combined.includes('retrieval') ||
      combined.includes('knowledge') || parsed?.hasKnowledgeBase) {
    return 'rag';
  }

  // Chatbot related
  if (combined.includes('chat') || combined.includes('bot') ||
      combined.includes('assistant') || parsed?.mode === 'chat') {
    return 'chatbots';
  }

  // Content related
  if (combined.includes('content') || combined.includes('write') ||
      combined.includes('seo') || combined.includes('copy') ||
      combined.includes('article') || combined.includes('blog')) {
    return 'content';
  }

  // Translation related
  if (combined.includes('translat') || combined.includes('language') ||
      combined.includes('multilingual') || combined.includes('i18n')) {
    return 'translation';
  }

  // Data related
  if (combined.includes('data') || combined.includes('analys') ||
      combined.includes('chart') || combined.includes('csv') ||
      combined.includes('excel') || combined.includes('report')) {
    return 'data';
  }

  // Automation related
  if (combined.includes('automat') || combined.includes('workflow') ||
      combined.includes('pipeline') || combined.includes('batch') ||
      combined.includes('schedule') || combined.includes('trigger')) {
    return 'automation';
  }

  // Development related
  if (combined.includes('code') || combined.includes('debug') ||
      combined.includes('develop') || combined.includes('review') ||
      combined.includes('github') || combined.includes('programming')) {
    return 'development';
  }
...
export function inferTags(filePath: string, parsed: ParsedDsl | null): string[] {
  const tags: string[] = [];
  const pathLower = filePath.toLowerCase();
