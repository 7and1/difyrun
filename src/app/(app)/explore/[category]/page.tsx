...
async function getCategoryWorkflows(
  ...
  const [{ workflows, total }, popularTags] = await Promise.all([
    ...
    getCachedPopularTags(categorySlug, 20),
  ]);
    ...
    <div className="container py-8">
      ...
      <div className="mb-8">
        <h1 className="text-3xl font-bold mb-2">
          {data.category.name} Workflows
        </h1>
        <p className="text-muted-foreground max-w-2xl">
          {data.category.description}. Browse {data.total} templates in this
          category.
        </p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-4 mb-8">
        ...
        >
          <SearchBar
            className="flex-1"
            placeholder={`Search ${data.category.name.toLowerCase()} workflows...`}
          />
        </Suspense>
